...
import { Search, Filter, Grid, List, Plus, MapPin, Eye, Users, Clock, Star, TrendingUp, Heart, Bell, ShoppingCart, Bookmark, Zap, Award, Target, ArrowRightLeft, DollarSign, BookOpen, Sparkles } from 'lucide-react';
...
export const ListingDashboardPage: React.FC<ListingDashboardPageProps> = ({
    ...
    setTimeout(() => {
      let results = mockBooks.filter(book => {
        ...
        return matchesQuery && matchesGrade && matchesSubject && matchesCondition && 
               matchesLocation && matchesExchange;
      });
      ...
      if (searchFilters.sortBy) {
        results.sort((a, b) => {
          switch (searchFilters.sortBy) {
            ...
            case 'savings': return ((b.originalPrice - b.price) / b.originalPrice) - ((a.originalPrice - a.price) / a.originalPrice);
            default: return 0;
          }
        });
      }
  ...
  const getUrgencyBadge = (book: Book) => {
    if (book.interestedBuyers > 5) return { text: 'Selling fast!', color: 'bg-red-500' };
        ...
        <div className="mb-8 space-y-6">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h2 className="text-2xl font-poppins font-bold mb-2">
                    🎉 You've helped {communityStats.userImpact.familiesHelped} families this month!
                  </h2>
                  <p className="text-lg opacity-90">
                    You've saved KES {communityStats.userImpact.totalSaved.toLocaleString()} and listed {communityStats.userImpact.booksListed} books
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-blue-800">
                    KES {(communityStats.communityImpact.totalSaved / 1000).toFixed(0)}K
                  </p>
                  <p className="text-xs text-blue-600">in {communityStats.communityImpact.location} this term</p>
                </div>
              </div>
            </div>
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-poppins font-semibold text-primary-700">Smart Filters</h3>
              <button
            ...
            {showFilters && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <select
                  value={filters.grade || ''}
                  onChange={(e) => setFilters(prev => ({ ...prev, grade: e.target.value ? parseInt(e.target.value) : undefined }))}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-neutral-600">
                  {isLoading ? 'Loading...' : `${searchResults.length} book${searchResults.length !== 1 ? 's' : ''} found`}
                </p>
              </div>
            ...
            {isLoading && (
              <div className="flex items-center justify-center py-12">
                <div className="text-center">
                  ...
                  <p className="text-neutral-600">Finding the best books for you...</p>
                </div>
              </div>
            )}
                ...
                {searchResults.map((book) => {
                  ...
                  return (
                    <div key={book.id} className="relative">
                      {urgencyBadge && (
                        <div className={`absolute top-2 left-2 z-10 ${urgencyBadge.color} text-white text-xs px-2 py-1 rounded-full font-medium animate-pulse`}>
                          {urgencyBadge.text}
                        </div>
                      )}
                      <BookCard
          ...
          <div className="space-y-6">
            ...
            <div className="card">
              <div className="flex items-center space-x-2 mb-3">
                ...
                <h4 className="font-poppins font-semibold text-primary-700">Badge Progress</h4>
              </div>
              <div className="space-y-3">
                <div>
                  <div className="flex items-center justify-between text-sm mb-1">
                    <span className="text-neutral-600">Power Seller</span>
                    <span className="font-semibold text-primary-700">17/20 sales</span>
                  </div>
                  <div className="w-full bg-neutral-200 rounded-full h-2">
                    <div className="bg-gradient-to-r from-gold-500 to-gold-600 h-2 rounded-full" style={{ width: '85%' }}></div>
                  </div>
                  <p className="text-xs text-gold-600 mt-1">3 more sales to unlock!</p>
                </div>
              </div>
            </div>
            ...
            <div className="card">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <h4 className="font-poppins font-semibold text-primary-700">Trending in Your Area</h4>
              </div>
              <div className="space-y-3">
                {['Grade 5 Mathematics', 'Grade 6 Science', 'Grade 4 English'].map((book, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-neutral-50 rounded-lg">
                    <div>
                      <p className="text-sm font-medium text-primary-700">{book}</p>
                      <p className="text-xs text-neutral-600">{5 - index} new listings</p>
                    </div>
